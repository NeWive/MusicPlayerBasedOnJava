...
public class javaConnectSQLite {
    ...
    public static Vector<Song> getAllSongs() {//查询所有歌曲
        ...
        try {
            ...
            while(result.next()){
                Song song_temp = new Song(result.getInt("musicId"),result.getString("musicname"),result.getInt("sheetid"),result.getString("md5"),result.getString("path"),"newive");
                songs.add(song_temp);
            }
    ...
    public static Vector<Song> searchSong(String musicname) throws SQLException {//查找音乐
        ...
        try {
            ...
            result = preparedStatement.executeQuery();
            while(result.next()){
                songs.add(new Song(result.getInt("musicId"), result.getString("musicname"), result.getInt("sheetid"), result.getString("md5"), result.getString("path"), "newive"));
            }
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (SQLException e) {
